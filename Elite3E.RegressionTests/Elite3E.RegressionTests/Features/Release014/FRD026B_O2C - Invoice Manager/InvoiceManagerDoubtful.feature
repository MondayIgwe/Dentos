@ignore
Feature: InvoiceManagerDoubtful
...
Examples:
	...
	| Ann Rose | Ann Rose | Ann Rose      | Singapore | Miscellaneous | SG Output Domestic Zero | 500.00       | at_BOA_Group    | SGD - Singapore Dollar | Lara Moon | Dentons Rodyk & Davidson LLP |
@ft @pipelineTest
Examples:
	| User     | Client   | FeeEarnerName | Office         | ChargeType    | TaxCode                    | ChargeAmount | ChargeTypeGroup | Currency            | PayorName | DefaultOperatingAlias          |
...
Scenario: 040 I want to view Doubtful invoices using Invoice Manager Matter
	...
	Then I set status
		| Status   | Office         | Email                   |
		| Doubtful | London (UKIME) | Disputed@dentons.global |
	And I can submit the record
	Then I want to view only 'Doubtful' invoices
	And I want to search for results
	Then I want to see only 'Doubtful' invoices
